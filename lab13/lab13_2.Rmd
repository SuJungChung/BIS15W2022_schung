...
server <- function(input, output, session) {
  ...
  ggplot(penguins, aes_string(x = input$x, y = input$y, color="species")) + 
      geom_point(size=input$pointsize) +  #users able to adjust the size
      theme_light()
    })
}
...
NOTES: The box function can help us separate the different parts, and it is part of the UI side
...
NOTE:
Want an app that allows users that select the population
...
ui <- dashboardPage(
  ...
  dashboardBody(
    fluidRow(
     ...
     box(title = "Wolf Male and Female Populations", width = 7,
