...
NOTES:
csv files can be imported and read everywhere
txt files are similar thing going on, but csv files are more frequently used
make sure they are all csv files
...
class(spider_list[[3]]) #tells us information on the file
...
str(spider_list[[23]]) # can use all the summary functions that you normally would use but use [[]]
```
...
```{r}
names_list <- strsplit(names, split = " .csv") #removing the space and the .csv
names_list
```

```{r}
names_vec <- unlist(names_list) #making a vector
names_vec
```

```{r}
names(spider_list) <- names_vec #use the vector and make the name of the spider list
names(spider_list)
```
...
inner_exampleDF #drops out the stuff that is missing stuff, but maybe you don't want incomplete data
```
...
left_exampleDF #4 is not there becuase you are joining to the left, the first data frame
#customer 3 is not in the second data frame, it doesn't have data so it is an NA
#matching everything to table_A
```
...
right_exampleDF #opposite of left join
#matching everythign to table_B
```

`full_join`
```{r}
full_exampleDF <- full_join(table_A, table_B, by="customer_ID")
full_exampleDF #puts everything together and accept the fact that the data has NA and work with the NA after
```
...
semi_exampleDF #like the first interjoin he showed us, where only fully complete data is shown
```
...
anti_exampleDF #going to provide the things that are missing in table_B
#things that aren't represented in both, this is the way to see the differences in the data tables
```
...
```{r}
spiders_with_locs <- 
  full_join(spiders_all, spiders_locs, by=c("Accession")) #full join tells us NA if there are errors with merging
...
```{r}
day <- today()
day #year-month-day, you want it to be put in like this
```

```{r}
str(day) #structure and can see the format
```
...
```{r}
dmy(spiders_with_locs$Date) #lubridate package
```

If you want to use base R, then here is the format.
```{r}
as.Date(spiders_with_locs$Date, format = "%d/%m/%y") #this is base R, not using the lubridate package
```
`
There are many different options for working with dates and datetimes in lubridate including `ymd`, `mdy`, `dmy`, `mdy_hms`, `ymd_hms`. 
```{r}
dateformat1 <- "20200922" #ways to convert the data
